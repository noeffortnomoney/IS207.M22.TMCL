<?php
    class checkoutmodel extends database{
        ...
        function AddOrder($id_user,$name,$phone,$address,$total){
            $sql = "INSERT INTO order_product VALUES ('', $id_user, current_time(), '$name', '$address', '$phone', 'Chờ Xử Lý',$total,'false')";
        ...
        function GetProductPay($id){
            $sql = "SELECT pay FROM product WHERE id = $id";
            $query = $this->conn->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
            return $result[0]["pay"];
        ...
        function GetHistotyOrder($id){
            $sql = "select * from order_product where user_id = $id";
            $query = $this->conn->prepare($sql);
            $query->execute();
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
            return $result;
