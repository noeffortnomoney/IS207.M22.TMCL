<?php
    ...
    class productmodel extends database{
        ...
        function AddProduct($name,$price,$img,$quantity,$decsription,$company,$id_category,$name_category,$sale){
            $sql = "insert into product values ('','$name','$price','$img',$quantity,'$decsription','$company',current_time(),'',$id_category,'$name_category',0,$sale)";
        ...
        function GetProductPage($id,$limit,$offset){
            $sql = "SELECT * FROM product where category_id = $id ORDER BY 'id' ASC LIMIT $limit OFFSET $offset";
            $query = $this->conn->prepare($sql);
            $query->execute();
